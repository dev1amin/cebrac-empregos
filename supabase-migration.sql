...
CREATE POLICY "empresas_public_read" ON empresas FOR
SELECT USING (status = 1);

CREATE POLICY "vagas_public_read" ON vagas FOR
SELECT USING (status = 1);

CREATE POLICY "noticias_public_read" ON noticias FOR
SELECT USING (status = 1);
